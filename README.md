...
> 详细设计文档：[Raft 共识算法实现详解](docs/RAFT.md)
...
> 详细设计文档：[LSM 树存储引擎详解](docs/LSM.md)
...
- **查看性能测试文档**:
 详见 [性能测试报告](docs/PERFORMANCE.md)，包含以下内容：
  - 全面的性能测试数据（单元测试、E2E、Benchmark、10 分钟长稳测试）
  - 16 项已完成的性能优化详解（含 Raft 三阶段锁、LSM 编码优化等）
  - 与优化前的对比数据（写入吞吐 6.2x 提升）
  - 当前架构瓶颈分析与优化路线图
- 存储后端性能对比
- 网络传输层性能对比
- 配置优化建议
