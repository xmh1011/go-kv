...
func NewSSTableWithLevel(level int, rootPath string) *SSTable {
	table := NewSSTable()
	table.level = level
	table.filePath = sstableFilePath(table.id, level, rootPath)
	return table
}
...
func (t *SSTable) DecodeFrom(filePath string) error {
	...
	if err = t.Header.DecodeFrom(file); err != nil {
		log.Errorf("[SSTable] decode Header from file %s error: %s", filePath, err.Error())
		return fmt.Errorf("decode Header failed: %w", err)
	}

	if err = t.FilterBlock.DecodeFrom(file); err != nil {
		log.Errorf("[SSTable] decode FilterBlock from file %s error: %s", filePath, err.Error())
		return fmt.Errorf("decode FilterBlock failed: %w", err)
	}

	// 定位到文件末尾，读取 Footer
	if err = t.DecodeFooterFrom(file); err != nil {
		log.Errorf("[SSTable] decode Footer from file %s error: %s", filePath, err.Error())
	...
	if _, err = file.Seek(t.Footer.IndexHandle.Offset, io.SeekStart); err != nil {
		log.Errorf("[SSTable] seek to index block position in file %s error: %s", filePath, err.Error())
	...
	if err = t.IndexBlock.DecodeFrom(file, t.Footer.IndexHandle.Size); err != nil {
		log.Errorf("[SSTable] decode IndexBlock from file %s error: %s", filePath, err.Error())
...
func (t *SSTable) DecodeFooterFrom(file *os.File) error {
	...
	if err != nil {
		log.Errorf("[SSTable] get file info for %s error: %s", t.filePath, err.Error())
		return fmt.Errorf("get file info failed: %w", err)
	}
	if _, err = file.Seek(fileInfo.Size()-block.FooterSize, io.SeekStart); err != nil {
		log.Errorf("[SSTable] seek to footer position in file %s error: %s", t.filePath, err.Error())
		return fmt.Errorf("seek to footer position failed: %w", err)
	}
	if err = t.Footer.DecodeFrom(file); err != nil {
		log.Errorf("[SSTable] decode Footer from file %s error: %s", t.filePath, err.Error())
		return fmt.Errorf("decode Footer failed: %w", err)
	}
...
func (t *SSTable) GetDataBlockFromFile(path string) ([]kv.KeyValuePair, error) {
	...
	if err != nil {
		log.Errorf("[SSTable] open file %s error: %s", path, err.Error())
		return nil, fmt.Errorf("open file error: %w", err)
	}
	defer func(file *os.File) {
		err := file.Close()
		if err != nil {
			log.Errorf("[SSTable] close file %s error: %s", path, err.Error())
		}
	}(file)

	if err = t.DecodeDataBlock(file); err != nil {
		log.Errorf("[SSTable] decode DataBlock from file %s error: %s", path, err.Error())
...
func (t *SSTable) GetKeyValuePairs() ([]kv.KeyValuePair, error) {
	...
	if t.DataBlock.Len() == 0 || t.IndexBlock.Len() == 0 {
		log.Warnf("[SSTable] SSTable %s has no data or index entries", t.filePath)
		return nil, nil
	}
	if t.DataBlock.Len() != t.IndexBlock.Len() {
		log.Errorf("[SSTable] SSTable %s has mismatched DataBlock and IndexBlock entries", t.filePath)
...
func (t *SSTable) GetValueByOffset(offset int64) (kv.Value, error) {
	...
	if err != nil {
		log.Errorf("[SSTable] open file %s error: %s", t.filePath, err.Error())
		return nil, fmt.Errorf("open file error: %w", err)
	}
	defer func(file *os.File) {
		err := file.Close()
		if err != nil {
			log.Errorf("[SSTable] close file %s error: %s", t.filePath, err.Error())
		}
	}(file)

	if _, err = file.Seek(offset, io.SeekStart); err != nil {
		log.Errorf("[SSTable] seek to offset error: %s", err.Error())
		return nil, fmt.Errorf("seek to offset failed: %w", err)
	}

	value := kv.Value{}
	if err = value.DecodeFrom(file); err != nil {
		log.Errorf("[SSTable] decode value error: %s", err.Error())
...
func (t *SSTable) Remove() error {
	if err := os.Remove(t.filePath); err != nil {
		log.Errorf("[SSTable] remove file %s error: %s", t.filePath, err.Error())
		return err
	}
	return nil
}
