...
// When data is written, it is first appended to the Write-Ahead Log (WAL) for durability,
...
func NewMemTable(id uint64, walPath string) *MemTable {
	...
	if err != nil {
		log.Errorf("[MemTable] Error creating new WAL: %s", err.Error())
		panic(fmt.Errorf("error creating new WAL: %w", err))
	}
	log.Debugf("[MemTable] Created new MemTable with ID: %d", id)
	return &MemTable{
		id:      id,
...
func (t *MemTable) Insert(pair kv.KeyValuePair) error {
	...
	if t.wal != nil {
		if err := t.wal.Append(pair); err != nil {
			log.Errorf("[MemTable] Error appending %+v to WAL: %s", pair, err.Error())
	...
	if ok := t.entries.Delete(key); ok {
		...
		if t.wal != nil {
			if err := t.wal.Append(kv.KeyValuePair{Key: key, Value: nil}); err != nil {
				log.Errorf("[MemTable] Error delete key %s from WAL: %s", key, err.Error())
...
func (t *MemTable) RecoverFromWAL(fileName string, walPath string) error {
	var err error
	t.id, err = utils.ExtractIDFromFileName(fileName)
	if err != nil {
		log.Errorf("[MemTable] Invalid WAL file: %s, err: %s", fileName, err.Error())
		return fmt.Errorf("invalid WAL file %s: %w", fileName, err)
	}

	pairs := make([]kv.KeyValuePair, 0)
	t.wal, err = wal.Recover(filepath.Join(walPath, fileName), func(pair kv.KeyValuePair) {
		pairs = append(pairs, pair)
	})
	if err != nil {
		log.Errorf("[MemTable] Recover WAL %s failed: %s", fileName, err.Error())
...
// Close closes the WAL file associated with the MemTable.
