...
#### TestLongRunning_10Min_Comprehensive 综合测试结果

| 指标 | 值 | 说明 |
|------|------|------|
| 测试时长 | 10m0s | 完整 10 分钟测试 |
| 总操作数 | 62,910,548 | 包含成功和失败 |
| 成功操作数 | 102,673 | 实际成功的请求 |
| 失败操作数 | 62,807,875 | 因超时等原因失败 |
| 成功率 | 0.16% | **极低，需要优化** |
| 错误率 | 99.84% | 高负载下的失败率 |
| 总吞吐量 | 171.12 ops/sec | 成功请求的吞吐量 |
| Leader 切换次数 | 18 | 频繁切换影响性能 |
| 数据一致性 | true | 即使高失败率下仍保持一致 |
...
**关键发现**:
1. **成功率极低 (0.16%)**: 在高并发场景下，大部分请求因超时失败
2. **锁竞争严重**: 大量 goroutine 等待 Raft.mu 锁
3. **Leader 频繁切换**: 18 次切换导致请求失败
4. **数据一致性保持**: 即使在高失败率下，数据仍然一致

**性能瓶颈分析**:
- Raft 单一全局锁导致严重竞争
- AppendEntries RPC 超时（2 秒在高负载下不足）
- 选举风暴导致 Leader 不稳定
