...
|------|--------|
| 总吞吐量 | **578.33 ops/sec** |
| 写入吞吐量 | 346.98 ops/sec |
| 读取吞吐量 | 144.50 ops/sec |
| 删除吞吐量 | 86.89 ops/sec |
...
| 时间点 | 总操作数 | 成功操作 | 吞吐量 (ops/sec) |
|--------|----------|----------|------------------|
| 30s | 34,986 | 34,984 | 1,166.08 |
| 1min | 59,163 | 59,160 | 986.00 |
| 2min | 100,138 | 100,134 | 834.43 |
| 3min | 138,046 | 138,042 | 766.90 |
| 4min | 183,691 | 183,687 | 765.36 |
| 5min | 209,098 | 209,092 | 696.97 |
| 6min | 247,582 | 247,574 | 687.69 |
| 7min | 291,386 | 291,377 | 693.75 |
| 8min | 302,153 | 302,139 | 629.46 |
| 9min | 333,401 | 333,379 | 617.37 |
| 10min | 337,512 | 337,485 | 592.08 |

**观察结论**:
- 初始吞吐量较高 (1166 ops/sec)，随数据量增加逐渐稳定
- 稳态吞吐量约 580-680 ops/sec
- 10 分钟内成功处理 34 万+ 操作
- 发生 3 次 Leader 切换，集群自动恢复
- 错误率极低 (0.0078%)，系统稳定可靠
...
| 指标 | 优化前 | 优化后 | 提升幅度 |
|------|--------|--------|---------|
| WriteHeavy 吞吐量 | 19 ops/sec | **456 ops/sec** | **24x** |
| WriteHeavy P99 延迟 | 2.62s | **156ms** | **-94%** |
| Comprehensive 吞吐量 | 37 ops/sec | **732 ops/sec** | **20x** |
| Comprehensive P99 延迟 | 1.94s | **102ms** | **-95%** |

#### 测试结果汇总

| 测试场景 | 总操作数 | 成功率 | 吞吐量 | P50 | P95 | P99 | Leader切换 |
|---------|---------|--------|--------|-----|-----|-----|-----------|
| Comprehensive | 439,353 | **100%** | 732 ops/sec | 6.3ms | 40ms | 102ms | 1 |
| WriteHeavy | 273,433 | **100%** | 456 ops/sec | 7.7ms | 52ms | 156ms | 1 |
| MixedWithFailures | 547,725 | **100%** | 913 ops/sec | 2.7ms | 16ms | 40ms | 1 |
| **ReadHeavy** | 97,298,146 | **100%** | **162,154 ops/sec** | 2µs | 63µs | 438µs | 0 |
| DeleteStress | 333,855 | **99.98%** | 556 ops/sec | 6.7ms | 39ms | 135ms | 1 |

#### 优化内容

1. **限制单次日志发送数量为 32 条**
   - 原来平均发送 588 条，现在最多 32 条
   - 减少 95% 的日志读取开销

2. **减少元数据写入次数**
   - 移除不必要的 FirstIndex 更新
   - 减少 33% 的元数据写入
...
1. **写入性能大幅提升**: 从 19 ops/sec 提升到 456 ops/sec
2. **延迟显著降低**: P99 从 2.6s 降低到 156ms
3. **成功率接近 100%**: 所有测试成功率 ≥ 99.98%
4. **读取性能优秀**: 162k ops/sec，得益于 Lease Read 机制
