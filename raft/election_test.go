...
func TestStartElection(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			peerIds: []int{2, 3},
			setupMocks: func(s *storage.MockStorage, tr *transport.MockTransport, sm *storage.MockStateMachine, stateChangeCh chan string, wg *sync.WaitGroup) {
				// 1. Pre-Vote 阶段: getLastLogInfoForElection uses cachedLastLogIndex, only GetEntry needed
				...
				gomock.InOrder(
					...
					// 获取日志 (only GetEntry, cachedLastLogIndex used for index)
					s.EXPECT().GetEntry(uint64(10)).Return(&param.LogEntry{Term: 5, Index: 10}, nil),
				)
		...
		},
		{
			...
			peerIds: []int{2, 3},
			setupMocks: func(s *storage.MockStorage, tr *transport.MockTransport, sm *storage.MockStateMachine, stateChangeCh chan string, wg *sync.WaitGroup) {
				// 1. Pre-Vote 阶段: cachedLastLogIndex used, only GetEntry needed
				s.EXPECT().GetEntry(uint64(10)).Return(&param.LogEntry{Term: 5, Index: 10}, nil)
			...
			verify: func(t *testing.T, r *Raft, stateChangeCh chan string, wg *sync.WaitGroup) {
				...
				r.mu.Lock()
				defer r.mu.Unlock()
				assert.Equal(t, Follower, r.getState(), "expected state to revert to Follower after timeout")
			},
		},
		{
			...
			setupMocks: func(s *storage.MockStorage, tr *transport.MockTransport, sm *storage.MockStateMachine, stateChangeCh chan string, wg *sync.WaitGroup) {
				...
				gomock.InOrder(
					...
					// 获取日志信息 (only GetEntry, cachedLastLogIndex used for index)
					s.EXPECT().GetEntry(uint64(10)).Return(&param.LogEntry{Term: 5, Index: 10}, nil),
			...
			verify: func(t *testing.T, r *Raft, stateChangeCh chan string, wg *sync.WaitGroup) {
				r.mu.Lock()
				defer r.mu.Unlock()
				assert.Equal(t, Follower, r.getState(), "State should revert to Follower")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r.cachedLastLogIndex = tt.initialState.cachedLastLogIndex
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(mockStore *storage.MockStorage) {
				// 期望 isLogUpToDate 会检查本地日志 (cachedLastLogIndex used, only GetEntry needed)
		...
		},
		{
			...
			mockSetup: func(mockStore *storage.MockStorage) {
				gomock.InOrder(
					...
					// 期望2: isLogUpToDate 检查日志 (cachedLastLogIndex used, only GetEntry needed)
					mockStore.EXPECT().GetEntry(uint64(10)).Return(&param.LogEntry{Term: 5, Index: 10}, nil),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := &Raft{
				...
				store:              mockStore,
				cachedLastLogIndex: tt.initialState.cachedLastLogIndex,
				mu:                 sync.Mutex{},
